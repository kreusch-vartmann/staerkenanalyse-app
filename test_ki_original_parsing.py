...
def test_ki_original_parsing():
    ...
    with app.app_context():
        ...
        participant = Participant.query.filter(
            Participant.ki_raw_response.isnot(None),
            Participant.ki_raw_response != ''
        ).first()
        ...
        try:
            ...
            if 'sk_ratings' in ki_original and isinstance(ki_original['sk_ratings'], dict):
                print("\nsk_ratings (first 3 items):")
                for i, (key, value) in enumerate(list(ki_original['sk_ratings'].items())[:3]):
                    print(f"  {key}: {value}")
            
            if 'vk_ratings' in ki_original and isinstance(ki_original['vk_ratings'], dict):
                print("\nvk_ratings (first 3 items):")
                for i, (key, value) in enumerate(list(ki_original['vk_ratings'].items())[:3]):
                    print(f"  {key}: {value}")
            
            if 'ki_texts' in ki_original and isinstance(ki_original['ki_texts'], dict):
                print("\nki_texts keys:")
                for key in ki_original['ki_texts'].keys():
                    text_preview = ki_original['ki_texts'][key][:100] if isinstance(ki_original['ki_texts'][key], str) else str(ki_original['ki_texts'][key])
