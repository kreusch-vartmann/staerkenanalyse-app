...
FAKE_FIRST_NAMES = [
    "Anna", "Max", "Sophie", "Leon", "Emma", "Paul", "Mia", "Lukas",
    "Hannah", "Felix", "Lena", "Jonas", "Laura", "Tim", "Marie", "David"
]

FAKE_LAST_NAMES = [
    "Schmidt", "Müller", "Weber", "Fischer", "Wagner", "Becker", "Hoffmann",
    "Koch", "Schulz", "Meyer", "Schneider", "Richter", "Zimmermann", "Braun",
    "Krüger", "Wolf", "Schäfer", "Keller", "Schulze", "Berger"
]
...
{{"social": "Beobachtung SK", "verbal": "Beobachtung VK"}}"""
    ...
    try:
        ...
        click.echo(f"   📝 Beobachtungen generiert: SK={len(result.get('social',''))//10} Words, VK={len(result.get('verbal',''))//10} Words")
        return result
    except Exception as e:
...
def _create_group(index):
    ...
    beobachter_name = f"{random.choice(FAKE_FIRST_NAMES)} {random.choice(FAKE_LAST_NAMES)}"
...
@click.option('--groups', default=2, help='Anzahl zu erstellender Gruppen (Standard: 2)')
@click.option('--participants', default='8-10', help='Teilnehmer-Range pro Gruppe (z.B. "8-10", Standard: 8-10)')
@click.option('--clear', is_flag=True, help='Alle Daten vor Generierung löschen (mit Bestätigung)')
@with_appcontext
def generate_test_data_command(groups, participants, clear):
    ...
    _generate_test_data(
        num_groups=groups,
        participants_range=(min_p, max_p),
