...
def import_participants_from_export(file, format='xlsx'):
    ...
    try:
        ...
        schema_version = df['_export_schema_version'].iloc[0] if '_export_schema_version' in df.columns else "unknown"
        
        # Version-spezifische Import-Logik
        if schema_version == "1.0" or schema_version == "unknown":
            return _import_schema_v1_0(df)
        else:
            return False, f"Unbekannte Schema-Version: {schema_version}. Bitte App aktualisieren.", 0
...
def _import_schema_v1_0(df):
    ...
    for _, row in df.iterrows():
        try:
            ...
            participant.observations = json.dumps(observations) if any(observations.values()) else None
            ...
            sk_ratings = {
                ...
                "results_orientation": _safe_numeric(row.get('SK Ergebnisorientierung')),
            }
            participant.sk_ratings = json.dumps(sk_ratings) if any(sk_ratings.values()) else None
            ...
            participant.vk_ratings = json.dumps(vk_ratings) if any(vk_ratings.values()) else None
            ...
            participant.ki_texts = json.dumps(ki_texts) if any(ki_texts.values()) else None
