...
from decorators import permission_required, group_access_required, participant_access_required
...
def upload_company_logo(group_id):
    ...
    if len(file.read()) > MAX_LOGO_SIZE:
        file.seek(0)
        return (
            jsonify({"error": f"Datei zu groß (max {MAX_LOGO_SIZE//1024//1024}MB)"}),
            400,
        )
    ...
    # Speichere in DB
    new_logo = CompanyLogo(logo_path=filepath, filename=filename, is_active=True)
    db.session.add(new_logo)
    db.session.commit()

    return jsonify({"success": True, "logo_id": new_logo.id, "filepath": filepath})
...
def upload_client_logo(group_id):
    ...
    # Speichere in DB
    new_logo = ClientLogo(group_id=group_id, logo_path=filepath, filename=filename)
    db.session.add(new_logo)
    db.session.commit()
...
def configure_report(group_id):
    ...
    if not config:
        ...
        default_config = {
            ...
            "info_page": {"enabled": True, "selected_block_ids": []},
            "toc_enabled": False,
        }
    ...
    explanation_blocks = ExplanationBlock.query.order_by(
    ...
    if request.method == "POST":
        ...
        if company_file and company_file.filename:
            if allowed_file(company_file.filename):
                ...
                new_logo = CompanyLogo(
                    logo_path=filepath, filename=filename, is_active=True
                )
                db.session.add(new_logo)
                db.session.flush()
        ...
        if client_file and client_file.filename:
            if allowed_file(client_file.filename):
                ...
                if old_logo:
                    db.session.delete(old_logo)
                new_logo = ClientLogo(
                    group_id=group_id, logo_path=filepath, filename=filename
                )
                db.session.add(new_logo)
                db.session.flush()
        ...
        selected_block_ids = [
            int(x) for x in request.form.getlist("explanation_block_ids") if x.isdigit()
        ]
        modules_config = {
            "cover_page": {
                "enabled": request.form.get("cover_page_enabled") == "on",
                "show_title": request.form.get("cover_page_show_title") == "on",
                "show_participant_name": request.form.get(
                    "cover_page_show_participant_name"
                )
                == "on",
                "show_group_name": request.form.get("cover_page_show_group_name")
                == "on",
                "show_date": request.form.get("cover_page_show_date") == "on",
                "show_company_logo": request.form.get("cover_page_show_company_logo")
                == "on",
                "show_client_logo": request.form.get("cover_page_show_client_logo")
                == "on",
                "title": request.form.get("cover_page_title", "Stärkenanalyse"),
                "subtitle": request.form.get(
                    "cover_page_subtitle", "Entwicklungsprofil"
                ),
            },
            "self_assessment": {
            ...
            "closing_page": {
                ...
                "show_signature_fields": request.form.get(
    ...
    # Unterschriften
    signature_fe = SignatureImage.query.filter_by(
        role="leitung_fe", is_active=True
    ).first()
    signature_se = SignatureImage.query.filter_by(
        role="leitung_se", is_active=True
    ).first()

    return render_template(
        "reports/configure.html",
        group=group,
        config=config,
...
def standalone_se_pdf(participant_id):
    ...
    try:
        ...
        traceback.print_exc()
        return redirect(url_for("analysis.manage_final_reports"))
...
def upload_signature():
    ...
    new_sig = SignatureImage(
        role=role, image_path=filepath, filename=filename, is_active=True
    )
