...
def add_explanation_block():
    ...
    breadcrumbs = [
        {"link": url_for("dashboard"), "text": "Dashboard"},
        {"link": url_for("explanation_blocks.manage_explanation_blocks"), "text": "Text"},
        {"text": "Neuer Textblock"}
    ]
    
    return render_template("explanation_block_form.html", breadcrumbs=breadcrumbs, block=None)


@explanation_blocks_bp.route("/explanation-block/edit/<int:block_id>", methods=["GET", "POST"])
def edit_explanation_block(block_id):
    ...
    if request.method == "POST":
        ...
        if not title:
            flash("Titel ist erforderlich.", "error")
            return redirect(url_for("explanation_blocks.edit_explanation_block", block_id=block_id))
    ...
    return render_template("explanation_block_form.html", breadcrumbs=breadcrumbs, block=block)


@explanation_blocks_bp.route("/explanation-block/delete/<int:block_id>", methods=["POST"])
