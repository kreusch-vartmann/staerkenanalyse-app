...
def check_database_py_usage():
    ...
    for py_file in python_files:
        if py_file.name == "database.py" or py_file.name == "check_legacy.py":
            continue
        if "venv" in str(py_file) or "__pycache__" in str(py_file) or ".github" in str(py_file):
            continue
...
def check_schema_sql():
    ...
    if "leitung TEXT" in schema_content and "leitung_fremdeinschatzung" in models_content:
        print("❌ DISKREPANZ: schema.sql hat 'leitung', models.py hat 'leitung_fremdeinschatzung' + 'leitung_selbsteinschatzung'")
